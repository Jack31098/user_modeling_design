...
1. OverviewUser Modeling & Retrieval System Design
...
Conditional Projection: Determine the search direction based on the current position ($v$) and intent ($Q$).

$$h_k = f_{\theta}^{(l)}( \text{Concat}(Q, v_{k}^{(l-1)}) )$$

Logit Computation: Compute the dot-product similarity with all $M$ entries in the codebook.
